...
class MultiEvalSolver_A_O(BaseSolver):
    ...
    def __init__(self, config, model=None, prefix_name=None):
        ...
        self.create_symlinks_to_imagenet(
            self.config.data.test.imagenet_o_folder,
            self.config.data.test.imagenet_o_root_dir,
        )
        all_wnids = [
            "n01440764",
            "n01443537",
        ...
        # setup_env() is now called at the beginning of __init__
        # self.setup_env()  # Moved to beginning of __init__
        # self.build_model()  # Called earlier if model is None
    ...
    def build_data(self):
        ...
        assert (
            getattr(self.config.data.test, "imagenet_a&o", False) is not False
        ...
        for imagenet_type in ["a", "o"]:
            ...
            if imagenet_type == "a":
                ...
                self.res_file = os.path.join(
                    ...
                    f"{imagenet_type}-results.txt.rank{self.dist.rank}",
                )
                writer = open(self.res_file, "w")
                confidence, correct, num_correct = self.get_predictions(
                    loader, net, mask, writer
                )
                ...
                self.res_file = os.path.join(
                    self.path.result_path,
                    f"{imagenet_type}-part1-results.txt.rank{self.dist.rank}",
                )
                writer = open(self.res_file, "w")
                confidence_in, correct, num_correct = self.get_predictions(
                    in_loader, net, mask, writer
                )

                self.res_file = os.path.join(
                    self.path.result_path,
                    f"{imagenet_type}-part2-results.txt.rank{self.dist.rank}",
                )
                writer = open(self.res_file, "w")
                confidence_out, correct_out, num_correct_out = self.get_predictions(
                    out_loader, net, mask, writer
                )
                if self.dist.rank == 0:
