...
class MultiEvalSolver_A_O(BaseSolver):
    ...
    def create_symlinks_to_imagenet(self, imagenet_folder, folder_to_scan):
        if not os.path.exists(imagenet_folder):
            ...
            for folder in folders_of_interest:
                ...
                os.symlink(os.path.join(path_prefix, folder), os.path.join(imagenet_folder, folder), target_is_directory=True)
    ...
    def build_data(self):
        ...
        assert getattr(self.config.data.test, "imagenet_a&o", False) is not False, \
            "This solver is only for imagenet a&o"

        if self.config.data.get('type', 'imagenet') == 'imagenet':
            self.val_data = build_imagenet_test_dataloader(self.config.data)
            self.naes = self.val_data['naes']
            self.noes = self.val_data['noes']
            self.val_loader_imagenet_o = self.val_data['val_loader_imagenet_o']
        else:
            raise NotImplementedError

    def get_predictions(self, loader, net=None, mask=None, writer=None):
        confidence = []
        correct = []
        num_correct = 0
        ...
        res = {'confidence': confidence, 'correct': correct, 'num_correct': num_correct}
        ...
        if self.dist.rank == 0:
            ...
            prefix = self.res_file.rstrip('0123456789')
            world_size = link.get_world_size()
            merged_file = prefix.rsplit('.', 1)[0] + '.all'
            merged_fd = open(merged_file, 'w')
            for rank in range(world_size):
                self.res_file = prefix + str(rank)
                assert os.path.exists(self.res_file), f'No such file or directory: {self.res_file}'
    ...
    def evaluate(self):
        ...
        for imagenet_type in ["a", "o"]:
            ...
            if imagenet_type is 'a':
                ...
                self.res_file = os.path.join(self.path.result_path, f'{imagenet_type}-results.txt.rank{self.dist.rank}')
                writer = open(self.res_file, 'w')
                confidence, correct, num_correct = self.get_predictions(loader, net, mask, writer)

                if self.dist.rank == 0:
                    acc = num_correct / len(loader.dataset)
                    self.logger.info('Result of Imagenet-a')
                    self.logger.info(f'Accuracy (%): {round(100 * acc, 4)}')
                    calibration_tools.show_calibration_results(confidence, correct)
                link.barrier()

            else:
                in_loader = self.val_loader_imagenet_o
                out_loader = self.noes
                mask = self.imagenet_o_mask

                self.res_file = os.path.join(self.path.result_path, f'{imagenet_type}-part1-results.txt.rank{self.dist.rank}')
                writer = open(self.res_file, 'w')
                confidence_in, correct, num_correct = self.get_predictions(in_loader, net, mask, writer)

                self.res_file = os.path.join(self.path.result_path, f'{imagenet_type}-part2-results.txt.rank{self.dist.rank}')
                writer = open(self.res_file, 'w')
                confidence_out, correct_out, num_correct_out = self.get_predictions(out_loader, net, mask, writer)
                if self.dist.rank == 0:
                    ...
                    auprs = measures[1]
                    fprs = measures[2]
                    self.logger.info('Result of Imagenet-o')
                    self.logger.info(f'AUPR {auprs}')
