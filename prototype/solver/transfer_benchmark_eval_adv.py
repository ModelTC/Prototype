...
class ClsSolver(BaseSolver):
    ...
    def setup_env(self):
        ...
        if hasattr(self.config.saver, "pretrain"):
            ...
            # # )
            # self.logger.info(
            #     f"target model: Recovering from {self.config.saver.pretrain.path_tgt}, keys={list(self.state_tgt.keys())}"
            # # )
    ...
    def evaluate(self, attack="none", eps=0):  # PGD-Linf, CW-L2, FGSM, AutoAttack
        self.logger.info(
            "Start evaluating on {} attack, eps={}".format(attack, str(eps))
        )
        self.model_src.eval()
        self.model_tgt.eval()
...
def main():
    ...
    config.model_src = model_name_dict[args.src_name]
    config.model_tgt = model_name_dict[args.tgt_name]
    ...
    if "efficientnet" in args.src_name.split("--")[0]:
        ...
        config.data.input_size = efficient_imgsize[
            int(args.src_name.split("--")[0][-1])
        ]
        config.data.test_resize = efficient_resize[
            int(args.src_name.split("--")[0][-1])
        ]
    # build solver
    prefix = (
