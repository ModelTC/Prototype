...
class ReparamSolver(BaseSolver):
    ...
    def setup_env(self):
        ...
        # add prefixes to pretrained deploy models
        if self.config.model.kwargs.deploy:
            self.state["model"] = {
                ("module." + k): v for k, v in self.state["model"].items()
            }
        # others
    ...
    def repvgg_model_convert(self):
        ckpt_training = {
            k.replace("module.", ""): v for k, v in self.state["model"].items()
        }
        train_model = model_entry(self.config.model)
        train_model.load_state_dict(ckpt_training)
        converted_weights = {}
        ...
        self.config.model.kwargs.deploy = True
        deploy_model = model_entry(self.config.model)
        for name, param in deploy_model.named_parameters():
            param.data = torch.from_numpy(converted_weights[name]).float()
        self.config.model.kwargs.deploy = False
            ...
            if curr_step > 0 and curr_step % self.config.saver.val_freq == 0:
                ...
                if self.dist.rank == 0:
                    if self.config.saver.save_many:
                        ckpt_train_name = (
                            f"{self.path.save_path}/ckpt_train_{curr_step}.pth.tar"
                        )
                        ckpt_deploy_name = (
                            f"{self.path.save_path}/ckpt_deploy_{curr_step}.pth.tar"
                        )
                    else:
                        ckpt_train_name = f"{self.path.save_path}/ckpt_train.pth.tar"
                        ckpt_deploy_name = f"{self.path.save_path}/ckpt_deploy.pth.tar"
                    ...
                    if self.ema is not None:
                        ...
                        self.deploy_state["ema"] = self.ema.state_dict()
                    torch.save(self.state, ckpt_train_name)

                    self.deploy_state["last_iter"] = 0
                    self.deploy_state["model"] = (
                        self.repvgg_model_convert().state_dict()
                    )
                    torch.save(self.deploy_state, ckpt_deploy_name)
...
def main():
    ...
    # build solver
    solver = ReparamSolver(args.config)
    # evaluate or train
    if args.evaluate:
