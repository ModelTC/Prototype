...
class _WarmUpLRScheduler(_LRScheduler):
    ...
    def _get_warmup_lr(self):
        if self.warmup_steps >= 2 and self.last_iter < self.warmup_steps:
            target_lr = (self.warmup_lr - self.base_lr) / (self.warmup_steps - 1) * (
                self.last_iter - 1
            ) + self.base_lr
...
class StepLRScheduler(_WarmUpLRScheduler):
    ...
    def __init__(
        ...
        if not list(lr_steps) == sorted(lr_steps):
            raise ValueError(
                "lr_steps should be a list of" " increasing integers. Got {}", lr_steps
            )
...
class CosineLRScheduler(_WarmUpLRScheduler):
    ...
    def _get_new_lr(self):
        ...
        target_lr = (
            self.min_lr
            + (self.warmup_lr - self.min_lr) * (1 + math.cos(math.pi * step_ratio)) / 2
        )
        scale = target_lr / self.base_lr
        return [scale * base_lr for base_lr in self.base_lrs]
...
class PolynomialLRScheduler(_WarmUpLRScheduler):
    ...
    def _get_new_lr(self):
        ...
        factor = (
            1 - (self.last_iter - self.warmup_steps) / float(self.max_iter)
        ) ** self.power
...
class CosineLREpochScheduler(_WarmUpLRScheduler):
    def __init__(
        ...
        super(CosineLREpochScheduler, self).__init__(
            optimizer, base_lr, warmup_lr, warmup_steps, last_iter
        )
        self.max_iter = max_iter
        self.min_lr = min_lr
    ...
    def _get_new_lr(self):
        ...
        target_lr = (
            self.min_lr
            + (self.warmup_lr - self.min_lr) * (1 + math.cos(math.pi * epoch_ratio)) / 2
        )
        scale = target_lr / self.base_lr
