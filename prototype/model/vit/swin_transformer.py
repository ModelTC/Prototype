...
class SwinTransformer(VisionTransformer):
    r"""Swin Transformer
    ...
    def init_norm_layer(self):
        self.norm_layer = (
            partial(self.norm_layer, eps=1e-6) if self.patch_norm else None
        )

    # override
    def init_block_type(self):
        self.block_type = SwinTransformerBlock
    ...
    def init_transformer(self):
        ...
        # build layers
        patches_resolution = to_2tuple(int(math.sqrt(self.patch_embed.num_patches)))
...
def swin_small(**kwargs):
    model = SwinTransformer(**kwargs, depths=[2, 2, 18, 2], drop_path_rate=0.3)
    return model
