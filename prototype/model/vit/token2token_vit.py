...
default_kwargs.update(
    {"embed_dim": 384, "depth": 14, "mlp_ratio": 3.0, "num_heads": 6, "qkv_bias": False}
)
    ...
    def init_pos_emb(self, num_patches, drop_rate):
        self.pos_embed = nn.Parameter(
            data=get_sinusoid_encoding(
                n_position=num_patches + 1, d_hid=self.embed_dim
            ),
            requires_grad=False,
        )
...
def T2t_vit_14(pretrained=False, **kwargs):  # adopt transformers for tokens to token
    default_kwargs.update({"tokens_type": "performer", "qk_scale": 384**-0.5})
    default_kwargs.update(kwargs)
    model = T2T_ViT(**default_kwargs)
    if pretrained:
        load_pretrained(
