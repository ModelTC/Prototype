...
class WeightNet(nn.Module):
    ...
    def forward(self, x, x_gap):
        ...
        x = F.conv2d(
            x, weight=x_w, stride=self.stride, padding=self.pad, groups=batch_size
        )
...
class WeightNet_DW(nn.Module):
    ...
    def __init__(self, inp, ksize, stride):
        ...
        # grouped fully connected operations (grouped FC)
        self.wn_fc1 = nn.Conv2d(
            inp_gap, self.M // self.G * inp, 1, 1, 0, groups=1, bias=True
        )
        self.sigmoid = nn.Sigmoid()
        self.wn_fc2 = nn.Conv2d(
            self.M // self.G * inp, inp * ksize * ksize, 1, 1, 0, groups=inp, bias=False
        )
