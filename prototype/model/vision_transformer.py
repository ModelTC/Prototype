...
class Encoder1DBlock(nn.Module):
    def __init__(
        ...
        self.attention = MultiHeadAttention(
            hidden_dim, heads, dropout, attention_dropout, qkv_bias, forward_type
        )
        self.norm2 = nn.LayerNorm(hidden_dim)
        self.feedforward = FeedForward(
            hidden_dim, mlp_dim, dropout, activation=activation
        )
...
class Encoder(nn.Module):
    def __init__(
        ...
        for d in range(depth):
            encoder_layer["encoder_{}".format(d)] = Encoder1DBlock(
                hidden_dim,
                mlp_dim,
                heads,
                dropout,
...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        self.embedding = nn.Conv2d(
            in_channel, hidden_dim, kernel_size=patch_size, stride=patch_size, padding=0
        )
    ...
    def forward(self, img):
        x = (
            self.embedding(img)
            .reshape(img.shape[0], -1, self.num_patches)
            .transpose(1, 2)
        )
        # x shape: [B, N, K]
