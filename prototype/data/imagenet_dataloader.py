...
def build_imagenet_train_dataloader(cfg_dataset, data_type="train"):
    ...
    if cfg_dataset["use_dali"]:
        ...
        assert (
            cfg_train["transforms"]["type"] == "STANDARD"
        ), "only support standard augmentation"
        dataset = Dataset(
        ...
        if isinstance(cfg_train["transforms"], list):
            transformer = build_transformer(
                cfgs=cfg_train["transforms"], image_reader=image_reader
            )
        else:
...
def build_imagenet_test_dataloader(cfg_dataset, data_type="test"):
    ...
    if cfg_dataset["use_dali"]:
        # NVIDIA dali preprocessing
        assert (
            cfg_test["transforms"]["type"] == "ONECROP"
        ), "only support onecrop augmentation"
        assert (
            cfg_test.get("imagenet_p", False) is False
        ), "dail not support for imagenet-p dataset"
        assert (
            cfg_test.get("imagenet_a&o", False) is False
        ), "dail not support for imagenet-a&o dataset"
        ...
        if isinstance(cfg_test["transforms"], list):
            transformer = build_transformer(
                cfgs=cfg_test["transforms"], image_reader=image_reader
            )
        else:
        ...
        if cfg_test.get("imagenet_c", False):
            ...
            dataset = VideoFolder(root=cfg_test["root_dir"], transform=transformer)
        elif cfg_test.get("imagenet_a&o", False):
            dataset_val_loader_imagenet_o = ImageFolder(
                root=cfg_test["imagenet_o_folder"], transform=transformer
            )
            dataset_a = ImageFolder(
                root=cfg_test["imagenet_a_root_dir"], transform=transformer
            )
            dataset_o = ImageFolder(
                root=cfg_test["imagenet_o_root_dir"], transform=transformer
            )
        else:
            dataset = ImageNetDataset(
                root_dir=cfg_test["root_dir"],
    ...
    if cfg_dataset["use_dali"]:
        ...
        cfg_test["sampler"]["kwargs"] = {
            "dataset": dataset_val_loader_imagenet_o,
            "round_up": False,
        }
        cfg_dataset["dataset"] = dataset_val_loader_imagenet_o
        sampler_dataset_val_loader_imagenet_o = build_sampler(
            cfg_test["sampler"], cfg_dataset
        )
        ...
        return {
            "type": "test",
            "val_loader_imagenet_o": val_loader_imagenet_o,
            "naes": loader_a,
            "noes": loader_o,
        }
    else:
...
def build_imagenet_search_dataloader(cfg_dataset, data_type="arch"):
    ...
    if cfg_dataset["use_dali"]:
        # NVIDIA dali preprocessing
        assert (
            cfg_search["transforms"]["type"] == "ONECROP"
        ), "only support onecrop augmentation"
        dataset = ImageNetDataset(
        ...
        if isinstance(cfg_search["transforms"], list):
            transformer = build_transformer(
                cfgs=cfg_search["transforms"], image_reader=image_reader
            )
        else:
    ...
    # build sampler
    assert (
        cfg_search["sampler"].get("type", "distributed_iteration")
