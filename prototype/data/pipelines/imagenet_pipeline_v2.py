...
class _PipelineBase(Pipeline):
    ...
    def __init__(self, custom_pipe, batch_size, num_threads, device_id, **kwargs):
        super(_PipelineBase, self).__init__(
            batch_size, num_threads, device_id, **kwargs
        )
...
class ImageNetTrainPipeV2(CustomPipeline):
    def __init__(self, data_root, data_list, sampler, crop, colorjitter=None):
        ...
        self.mc_input = ops.McReader(
            file_root=data_root, file_list=data_list, sampler_index=list(sampler)
        )
        ...
        if self.colorjitter is not None:
            ...
            self.rng_brightness = ops.Uniform(
                range=(1.0 - self.colorjitter[0], 1.0 + self.colorjitter[0])
            )
            self.rng_contrast = ops.Uniform(
                range=(1.0 - self.colorjitter[1], 1.0 + self.colorjitter[1])
            )
            self.rng_saturation = ops.Uniform(
                range=(1.0 - self.colorjitter[2], 1.0 + self.colorjitter[2])
            )
            self.rng_hue = ops.Uniform(
                range=(-self.colorjitter[3], self.colorjitter[3])
            )
    ...
    def define_graph(self):
        ...
        if self.colorjitter is not None:
            images = self.colorjit(
                images,
                brightness=self.rng_brightness(),
